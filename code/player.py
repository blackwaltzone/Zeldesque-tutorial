...
class Player(Entity):
	...
	def get_facing(self):
		...
		if self.direction.x == 0 and self.direction.y == 0:
			if not 'idle' in self.facing and not 'attack' in self.facing:
				self.facing = self.facing + '_idle'

		if self.attacking:
			self.direction.x = 0
			self.direction.y = 0
			if not 'attack' in self.facing:
				if 'idle' in self.facing:
					# override idle
					self.facing = self.facing.replace('_idle','_attack')
				else:
					self.facing = self.facing + '_attack'
		else:
			if 'attack' in self.facing:
				self.facing = self.facing.replace('_attack', '')
